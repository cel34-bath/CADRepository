...
try
	RC_result := timelimit(tlim*iter, Usage(RegularChains:-SemiAlgebraicSetTools:-CylindricalAlgebraicDecompose(L_RC,RegularChains:-PolynomialRing(rc_ordering),output = 'allcell',optimization = 'EC'),output = 'all',quiet = false,iterations = iter)):
...
qe_ordering := tonks:
try
QE_result := timelimit(tlim*iter, Usage(QE:-CylindricalAlgebraicDecompose(L_QE,variablestrategy = qe_ordering,propagateecs = false,useequations = 'single',usegroebner = false),output = 'all',quiet = false,iterations = iter)):

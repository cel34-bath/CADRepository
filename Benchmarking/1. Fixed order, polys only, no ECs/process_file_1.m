...
rc_ordering := vars:
try
	RC_result := timelimit(tlim*iter, Usage(RegularChains:-SemiAlgebraicSetTools:-CylindricalAlgebraicDecompose(L,RegularChains:-PolynomialRing(rc_ordering),output = 'allcell',optimization = false),output = 'all',quiet = false,iterations = iter)):
...
qe_ordering := vars:
try
	QE_result := timelimit(tlim*iter, Usage(QE:-CylindricalAlgebraicDecompose(L,variablestrategy = qe_ordering,propagateecs = false,useequations = none,usegroebner = false),output = 'all',quiet = false,iterations = iter)):
...
#PrintProjection(QE_result[output]):
#quit();